...
import {toWebRequest} from '../runtime/node.mjs';
...
test('Node adapter bounds request bodies before constructing a Web Request',async()=>{const request=new EventEmitter();request.method='POST';request.url='/test';request.headers={'content-type':'text/plain'};request.resume=()=>{};const converted=toWebRequest(request,{maxBodyBytes:4});request.emit('data',Buffer.from('oversized'));await assert.rejects(converted,error=>error.code==='REQUEST_BODY_TOO_LARGE');});
