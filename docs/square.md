...
The mutating Sandbox lifecycle test is opt-in. Set `COMMERCE_RUN_MUTATING_SANDBOX=true`, `SQUARE_ACCESS_TOKEN`, `SQUARE_LOCATION_ID`, `SQUARE_TEST_CUSTOMER_ID`, `SQUARE_TEST_CARD_ID`, and `SQUARE_TEST_PLAN_VARIATION_ID`, then run `npm run test:sandbox`. It creates one subscription with a stable idempotency key, repeats creation to prove provider deduplication, retrieves it, and schedules cancellation in cleanup. Use dedicated Sandbox objects only.
